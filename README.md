...
Сперва требуется выполнить команду `git status` это позволит узнать текущее состояние файлов, что было добавлено, какие находятся в не отслеживаемом состоянии, какие были изменены.

После этого необходимо добавить файлы в следующий коммит. Осуществляется это командой `git add <имя файла>`.
...
В данном файле можно указывать конкретные имена файлов которые мы хотим исключить от попадания в коммит. Например:
```c#
...
Осуществляется это командой `git pull`
